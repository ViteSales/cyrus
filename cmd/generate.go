...
import (
	...
	"github.com/vitesales/cyrus/tools/generate"
...
func init() {
	CyrusCmd.AddCommand(generateCmd)
...
func replacePoolDirInGoMod(poolDir string) {
	runCommand("go", "mod", "edit", "-replace", fmt.Sprintf("github.com/vitesales/cyrus-pool@v1.0.2=%s", poolDir))
}
...
func copyGoModReplaces(poolDir string) {
	...
	for _, repl := range replaces.Replace {
		if repl.Old.Path == "github.com/vitesales/cyrus-pool" {
			continue
		}
...
module github.com/vitesales/cyrus-pool
`))
...
func main() {
	var cyrusCmd = &cobra.Command{
		...
		Long: "Cyrus is an open source modular ERP written in Go. It is designed for high demand business data processing while being easily customizable",
	}
	cmd.SetCyrusFlags(cyrusCmd)

	var serverCmd = &cobra.Command{
		Use:   "server",
		Short: "Start the Cyrus server",
		Long: "Start the Cyrus server",
		Run: func(c *cobra.Command, args []string) {
			cmd.StartServer()
		},
	}
	cyrusCmd.AddCommand(serverCmd)
	...
	cyrusCmd.AddCommand(updateDBCmd)

	cobra.OnInitialize(cmd.InitConfig)

	if err := cyrusCmd.Execute(); err != nil {
		fmt.Println(err)
		os.Exit(-1)
	}
}
